...
export default function CMSEditPage() {
  ...
  const loadArticle = async () => {
    try {
      ...
      const article = await api.cms.getArticle(articleId, token);
  ...
  const handleSubmit = async (values: typeof form.values) => {
    ...
    try {
      ...
      await api.cms.update(articleId, {
        ...values,
      ...
      notifications.show({
        title: '更新失敗',
        message: error instanceof Error ? error.message : '無法更新文章',
        color: 'red',
      });
    } finally {
      setSaving(false);
